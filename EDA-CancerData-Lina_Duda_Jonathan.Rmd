...
In addition, we will look at a few variables of interest as a secondary analysis: Cancer Incident Count, Median Income and Povery Rate.
...
Our key variables in this investigation will be deathRate (target outcome variable) and several indpendent variables representing insurance coverage for counties' populations.    
...
```
Both from the plot and from the correlation value (-0.39) we can see that they're in agreement with our original hypothesis that mortality rates ared lower for the populations with higher percentage of private insurance coverage. 
...
We will also evalue how incident rate (which we assume is the number of people diagnosed with cancer annually) relates to mortality rate.
...
Taking a deeper look into another socieconomic variable might strenghten our hypothesis.  
...
Given the strong positive correlation between poverty levels and mortality rate, we should expore the relationship between poverty rates and insurance coverage.
...
In order to explain these counter-intuitive results, as well as the 'threshold effect' of private coverage, we decided to explore other variables that might directly influence these relationships, turning our attention to sociioeconomic factors, specifically poverty rate and median income.  
...
- Populations with higher percentage of poverty tend to have higher cancer death rates, and poverty conditions limitate the access to private health insurance coverage, being more dependent on the public alternative.  

Therefore, there is stronger evidence that social economic factors (income, poverty) are stronger factors in explaining the cancer death rates than health insurance per ce, being that the coverage by type of health insurance is also probably affected by these factors, explaining their opposite behaviors with death rates.
