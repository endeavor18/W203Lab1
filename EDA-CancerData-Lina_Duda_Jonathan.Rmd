...
```{r CheckData}
colSums(is.na(cancer.df)) # Check for NA Values in all columns
```
...
-------------------    -----------------------------------------------------------------------
We also want to bin the coverage variables when doing our analysis, introducing the following variables:
Variable Name          Description    
-------------------    -----------------------------------------------------------------------  
...
We are now going to evaluate the relation between the median income and the percentage of health insurance coverage by type.
#
```{r Sec_Analysis_1}
...
As expected, we have a high positive correlation between median income and the percentage of health insurance coverage.  
Let us check the boxplot by the binned median income and the binned percetage of private health insurance coverage.

```{r Sec_Analysis_2}

boxplot(PctPrivateCoverage ~ IncomeCat, data= cancer.df, main = "Percentage of Private Health Insurance Coverage by Median Income", xlab="Median Income", ylab="Private Coverage")
```
